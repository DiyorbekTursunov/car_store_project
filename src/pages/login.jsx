...
import { setTokenAdmin } from "../slices/loginAdmin"
const Login = () => {
    ...
    const fetchLogin = async (userData) => {
        ...
        try {
            ...
            if (data.status === 200 & data.data.msg === "Success") {
                ...
                dispach(setTokenAdmin(data.data.verifyAdmin))
                navigate('/modellari')
            } else {
                IsErrorFun(data.data.msg)
            }
            setloader(false)
        } catch (error) {
            IsErrorFun(error.data)
            setloader(false)
    ...
    function loginFun(e) {
        ...
        if (userDatail.email.length & userDatail.password.length) {
            ...
            IsErrorFun("Ma'lumot to'liq kiritilmagan")
        }
    }

    function IsErrorFun(errorValue) {
        setisError(true)
